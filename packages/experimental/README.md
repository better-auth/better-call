...
`src/plugins/capability.ts` (exported as `better-call/plugins/capability`, demoed in `test/capability-demo.ts`) builds security out of the primitives above. The model in one sentence:
...
Every served fn runs one check before its body: *was my caller authorized to make this call with this input?* (`validateCaller` in `src/plugins/capability.ts`):
